...
func NewEmailChannel(from, to, subject string, smtpConfig *EmailConfig) *EmailChannel {
	return &EmailChannel{
		BaseChannel: &BaseChannel{id: from + ":" + subject},
		from:        from,
		to:          to,
		subject:     subject,
...
// WriteChunk 收集内容，当收到 Done 块时发送邮件
func (ec *EmailChannel) WriteChunk(chunk StreamChunk) error {
	...
	if chunk.Content != "" {
		ec.body.WriteString(chunk.Content)
	}
	if chunk.ReasoningContent != "" {
		ec.body.WriteString(chunk.ReasoningContent)
	}
	if chunk.Done && !ec.done {
		ec.done = true
		return ec.sendEmail()
	}
	return nil
