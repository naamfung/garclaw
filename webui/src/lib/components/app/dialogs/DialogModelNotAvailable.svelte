...
<AlertDialog.Root {open} onOpenChange={handleOpenChange}>
        <AlertDialog.Content class="max-w-lg">
                ...
                <div class="space-y-3">
                        <div class="rounded-lg border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm">
                                <p class="font-medium text-amber-600 dark:text-amber-400">
                                        请求的模型：<code class="rounded bg-amber-500/20 px-1.5 py-0.5">{modelName}</code>
                                </p>
                        </div>

                        {#if availableModels.length > 0}
                                <div class="text-sm">
                                        <p class="mb-2 font-medium text-muted-foreground">选择一个可用模型：</p>
                ...
                <AlertDialog.Footer>
                        <AlertDialog.Action onclick={() => handleOpenChange(false)}>取消</AlertDialog.Action>
                </AlertDialog.Footer>
        </AlertDialog.Content>
</AlertDialog.Root>
