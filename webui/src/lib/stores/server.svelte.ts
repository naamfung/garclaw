 ...
 * - **Timeout Config**: Server timeout settings (shell, http, plugin, browser)
 */
class ServerStore {
        /**
         *
        ...
        needsSetup = $state(false); // 是否需要配置（API Key 未设置）
        timeoutConfig = $state<{ shell: number; http: number; plugin: number; browser: number } | null>(null);
        ...
        async fetch(): Promise<void> {
                ...
                const fetchPromise = (async () => {
                        try {
                                ...
                                // 检查是否需要配置
                                if (props && 'needs_setup' in props) {
                                        this.needsSetup = (props as any).needs_setup === true;
                                }
                                ...
                                try {
                                        ...
                                        if (configResp.ok) {
                                                ...
                                                if (configData.timeout) {
                                                        this.timeoutConfig = configData.timeout;
                                                }
                                        }
                                } catch (configError) {
                                        console.warn('Failed to fetch timeout config:', configError);
                                }
                        } catch (error) {
...
export const needsSetup = () => serverStore.needsSetup;
export const timeoutConfig = () => serverStore.timeoutConfig;
